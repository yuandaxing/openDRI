...
@login_required()
def new(request) :
    return TemplateResponse(request, 'users/edit.html',
                            {'methods': UserNotificationMethod.methods, 'empty_user_method' : UserNotificationMethod() })
...
def edit(request, id):
    try:
        ...
        return TemplateResponse(request, 'users/edit.html', 
                                {'item' : user,  'user_methods' : user_methods, 'methods': UserNotificationMethod.methods, 
...
def save(request):
    ...
    try :
        ...
        if int(request.POST['id']) > 0 :
            return HttpResponseRedirect(reverse('opendri.user.edit', None,
                                                [str(request.POST['id'])]))
        else :
            return HttpResponseRedirect(reverse('opendri.users.new'))
