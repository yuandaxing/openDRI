...
def new(request) :
    ...
    try :
        ...
        calendars = None
    return TemplateResponse(request, 'escalation/edit.html', 
                            {'calendars' : calendars, 
                             'users' : users })
@login_required()
def edit(request, id) :
...
def save(request):
    ...
    try :
        ...
        if int(request.POST['id']) > 0 :
            return HttpResponseRedirect(reverse('opendri.escalation.edit', None, [str(request.POST['id'])]))
        else :
            return HttpResponseRedirect(reverse('opendri.escalation.new'))
